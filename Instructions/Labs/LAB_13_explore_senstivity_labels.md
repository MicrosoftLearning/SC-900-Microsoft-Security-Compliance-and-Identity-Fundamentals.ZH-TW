...
1. 從資訊保護頁面，選取自動加上標籤。 請檢閱描述。 請注意，您可以建立自動套用標籤原則，將敏感度標籤自動套用至包含敏感性資訊的電子郵件訊息或 OneDrive 和 SharePoint 檔案。 我們的租用戶中尚未預先設定任何自動標籤原則。 若要建立新的自動標籤原則，請選取 [建立自動標籤原則]****。  您在此將逐步解說建立新原則的步驟。
...
1. 從頂端功能表列中，選取**敏感度**圖示，即麥克風圖示右邊的圖示 (視您的螢幕大小而定，您可能需要選取省略符號，然後選取敏感度)。 頁面頂端的黃色橫幅隨即出現，右側會顯示選取標籤按鈕。  選取 [選取標籤] **** 按鈕。  出現彈出式視窗。   選取 [選取標籤]****，接著從下拉式清單中選取 [機密-財務]****，然後選取 [確定]****。  
