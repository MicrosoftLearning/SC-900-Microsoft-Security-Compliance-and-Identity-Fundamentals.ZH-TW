...
1. 從頁面頂端選取 [新增]。 在 [新增輸入安全性規則] 的視窗中，指定下列設定：
    ...
    1. 服務：**RDP**
    1. 動作：**允許**
    1. 優先順序：**1000** 注意：編號較小的規則優先順序較高，應首先進行處理。
    1. 名稱：保留預設名稱，或建立自己的描述性名稱。
    1. 請注意頁面底部的警告符號。  我們只使用 RDP 進行測試，以及示範 NSG 的功能。
    1. 選取 [新增]

1. 規則佈建完成後，即會出現於輸入規則的清單中 (您可能需要重新整理畫面)。 在新增的規則上，您會看到警告符號。  如上所述，我們只使用 RDP 進行測試，以及示範 NSG 的功能。 選取新增的規則。
...
1. 系統會提示您提供認證。  輸入 [使用者名稱] 和 [密碼] (請參考實驗室指示面板上的 [資源] 索引標籤)。

1. 遠端桌面連線視窗開啟，顯示無法驗證遠端電腦的身分識別。  仍要繼續連線？  選取 [是]  。

1. 您現在已連線至 VM。 在這種情況下，您能夠連線至 VM，因為您建立的輸入流量規則允許透過 RDP 向 VM 傳送輸入流量。

1. 在 [歡迎] 畫面上幾秒鐘後，您會看到一個視窗可供選擇裝置的隱私權設定，選取 [接受]。  在 [網路] 視窗上，選取 [否]。

1. 啟動並執行 VM 後，從 VM 測試與網際網路的輸出連線能力。
...
1. 您應該位於 [SC900-WinVM | 網路] 頁面。 如果您先前關閉了瀏覽器索引標籤，請選取頁面頂端的藍色搜尋列並選取虛擬機器，接著選取 VM **SC900-WinVM**，然後選取 [網路]。
