...
若要建立 Microsoft Sentinel 的執行個體，您必須先建立 Log Analytics 工作區，用來儲存來自 Microsoft Sentinel 的資料。  擁有 Log Analytics 工作區之後，您可以建立執行個體 Microsoft Sentinel，並將 Log Analytics 工作區新增至其中。  在此作業中，您會執行每一個步驟。

1. 您應該在 Azure 服務的首頁。  如果沒有，請開啟 Microsoft Edge，然後在網址列中輸入 **portal.azure.com**，然後使用您的 Azure 入口網站系統管理員認證登入。
...
建立 Microsoft Sentinel 執行個體並指派給它的 Log Analytics 工作區後，負責支援 Microsoft Sentinel 的使用者必須擁有必要的許可權。  將必要的角色權限指派給指定的使用者，完成此作業。  在這項工作中，您將檢視可用的內建 Microsoft Sentinel 角色。
...
1. 從 Azure 服務首頁中，選取 [Microsoft Sentinel]，然後選取您建立 **的執行個體 SC900-Sentinel-workspace**。

1. 從左側導覽面板中，展開 **內容管理** 然後選取 內容 **中心**。
...
1. 從左側導覽面板中，展開 **[威脅管理** ]，然後探索威脅管理中列出的選項。
    1. 選取 [事件]****。  雖然找不到任何事件，但請檢閱 它**是什麼？**
    1. 選取 [ **搜捕]** ，然後檢閱 [ **搜捕 （預覽）]** 索引標籤中提供的資訊。
    1. 選取 [筆記本 **]**，然後檢閱 [ **它是什麼？]** 區段。
...
1. 從視窗的左上角，在藍色橫幅中，選取 **[Microsoft Azure**] 以返回 Azure 入口網站的首頁。

1. 登出並關閉所有開啟的瀏覽器索引標籤。

### 檢閱

在此實驗室中，您已逐步解說將 Microsoft Sentinel 連線到資料來源的步驟、設定活頁簿，以及逐步解說 Microsoft Sentinel 中可用的數個選項。
