...
1. 您現在位於 VM 設定的網路索引標籤中。  在NIC 網路安全性群組選項中，選取 [無]。 保留所有其他設定的預設值。
...
1. 退回 **概覽** 索引標籤。請注意，在頁面的右上角，您將看到「關聯：0 個子網路、1 個網路介面」」– 該介面現已指派給 NSG。 此外，向學員強調預設規則。 對於輸入，僅允許來自其他 Azure 虛擬網路和 Azure Load Balancer 的流量。 到 VM 的所有其他輸入流量都將遭到拒絕。 同時指出預設輸出規則。  僅允許到其他 VNet 的輸出流量和輸出網際網路流量。  作為示範的一部分，建議您花一分鐘的時間來顯示輸入流量遭到拒絕。
    ...
    1. 請注意，連接埠的必要條件未受到滿足。  若要允許滿足必要條件，則須設定輸入網路安全性規則，其目的地連接埠為 3389 且由 RDP 使用。  

1. 現在您想要建立新的規則，以允許輸入的 RDP 流量。  請指出，您不能刪除現有預設規則，您只能建立具有更高優先順序的新規則。 從左側導覽面板的設定下，選取 **網路**。  您目前位於 VM 的網路頁面。
1. 請確認已選取 [輸入連接埠規則] 索引標籤 (已加上底線)，然後選取 [新增輸入連接埠規則]，以建立允許輸入 RDP 流量的規則，設定如下：
    1. 來源：**任何**
    1. 來源連接埠範圍： **\***
    1. 目的地：**任何**
    1. 服務：**RDP**
...
1. 現在選取 [輸出連接埠規則] 索引標籤，然後檢閱預設規則。  從頁面頂端選取 [新增輸出連接埠規則]，然後與不同設定進行對話。  本人建議建立規則 – 以下設定會建立規則來拒絕輸出網際網路流量：
    1. 來源：**任何**
    1. 來源連接埠範圍： **\***
    1. 目的地：**服務標籤**
    1. 目的地服務標籤：**網際網路**
...
1. 現在退回 VM 並測試規則。  請在頁面頂端選取 [SC900-VM]。
