...
1. 如果這是您第一次造訪 Microsoft 365 Defender 入口網站，您可以取得快顯視窗以快速導覽。
...
1. 使用適用於雲端的 Microsoft 365 Defender 應用程式，可藉由設定應用程式連接器來直接連線至應用程式，讓您對雲端應用程式有更大的可見度與控制權。 從畫面右上角選取 [動作]****，然後選取 [Cloud Discovery 設定]****。  注意可用的設定。
...
1. 為了提供資料保護，適用於雲端的 MicrosoftDefender 應用程式可讓您檢視連線應用程式的所有檔案，例如儲存在 SharePoint 和 Salesforce 中的所有檔案。 從左側瀏覽窗格中，選取並探索 [使檔案]**** 選項。
    ...
    1. 從左側導覽面板中選取雲端應用程式底下的 [檔案] **** 以返回檔案。 您可能不會看到任何列出的項目，因為這可能需要數天的時間才能看到您的檔案，但值得說明的是，檔案列出後您可以依應用程式、擁有者、存取層級、檔案類型和相符的原則來篩選資料。 此外，您要從資料搜尋和匯出建立新原則。
...
1. 左側導覽面板中，選取 [首頁]**** 以返回 Microsoft 365 Defender 的首頁。
