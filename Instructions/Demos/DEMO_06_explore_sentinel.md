...
1. 在 Microsoft Sentinel 頁面上，您應該會看到列出的 Log Analytics 工作區。  這表示已建立 Microsoft Sentinel 實例，並已將 Log Analytics 工作區新增至其中。 如果沒有，請按照以下步驟操作。
...
1. 從 [存取控制] 頁面中，選取 [檢視我的存取權]****。  您很可能會看到已為此訂用帳戶設定的自訂角色指派，而且它會授與您必要的許可權。  此自訂角色是由提供實驗室環境的授權實驗室主機 （ALH） 所設定。 不過，為了進行示範最好展示 Sentinel 特定角色。  選取視窗右上角的 **X** 以關閉指派視窗。
